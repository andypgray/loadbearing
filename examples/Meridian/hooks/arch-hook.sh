...
# Meridian's paths. Same exit-code contract as the PowerShell variant: clean check -> 0 (the
# turn ends); a red rule -> 2 (the stop is refused, with the violation report on stderr so the
# agent reads the rule ID, reason, fix, and file:line and keeps working); LoadBearing's own
# error -> 1 (a non-blocking config problem, not an architecture violation the agent is told
# to "fix").
#
