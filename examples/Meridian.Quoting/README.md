...
Four layers run one direction, and the module map at the top of [AGENTS.md](AGENTS.md) says what each is for, rendered from the `.Purpose` each layer declares in the spec. The rules hold that shape; the subsystem was born conforming, so there is nothing to ratchet.
