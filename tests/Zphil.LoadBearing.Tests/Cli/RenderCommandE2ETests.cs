...
public sealed class RenderCommandE2ETests
{
    ...
    // The family rules' bullets, each spelled once because both appear on BOTH cells' cards — the placement
    // decision made visible, and two copies of a string would let one drift.
    private const string LeavesBullet =
        "- `layering/leaves-independent` — Each of the Reporting and Billing layers must not reference " +
        "the others. Reporting and billing are the two leaves of this solution; neither may grow a " +
        "dependency on the other.\n";

    private const string LeavesNotCircularBullet =
