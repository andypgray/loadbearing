...
{
    // The recovery line every rendered root block carries, spelled once so the two goldens below cannot
    // drift apart on it. Sibling RenderCommandE2ETests holds the same paragraph for its own goldens.
    ...
    [Fact]
    public void RootBlock_InstallRoute_NamesDotnetDnxNotBareDnx()
    {
        // Which lines the recovery paragraph carries, and where they sit, is already the golden's claim
        // above. This is the one thing the golden cannot make: an edit that reached for a bare `dnx` —
        // the spelling that needs the tool the paragraph exists for the reader not to have — would move
        // the golden with it and read green.
    ...
    public void ScopeCard_NonContainmentRule_Throws()
    {
        ...
        ArchRule enforceRule = Checker.Rule("naming/x");
