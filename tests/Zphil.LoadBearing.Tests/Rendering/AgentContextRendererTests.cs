...
public class AgentContextRendererTests
{
    ...
    // Built once: the model is immutable and the four gates below only render from it — three of them to
    // prove a glossary clause stays off, which is the same block read four ways.
    private static readonly ArchitectureModel DogfoodShapeModel = ArchModelBuilder.Build(DogfoodShapeSpec);
    ...
    public void RootBlock_CanonicalSample_MatchesGolden()
    {
        ...
        AgentContextRenderer.RootBlock(Checker.Canonical, "CanonicalSampleSpec")
            .ShouldBe(expected);
    }
    ...
    public void RootBlock_NoLayerSingleRuleSpec_OmitsLayersSection()
    {
        ...
        AgentContextRenderer.RootBlock(DogfoodShapeModel, "Zphil.LoadBearing.ArchSpec")
            .ShouldBe(expected);
    }

    [Fact]
    public void RootBlock_NamesTheInstallRoute_BothSpellings()
    {
        // The one committed line for the reader whose machine lacks the tool: a checkout can commit its
        // MCP wiring but not the install, so the first session on a fresh clone meets a configured
        // server that dies at launch, and nothing else in the block names the cure. Both spellings,
        // because the reader holds the branch: the installed command for the machine that has it, the
        // dnx launch for the one that does not.
        string block = AgentContextRenderer.RootBlock(Checker.Canonical, "CanonicalSampleSpec");

        int provenance = block.IndexOf("*Generated by `loadbearing render`", StringComparison.Ordinal);
        int recovery = block.IndexOf("If the `loadbearing` command is missing", StringComparison.Ordinal);
        int heading = block.IndexOf("## Architecture (LoadBearing)", StringComparison.Ordinal);
        provenance.ShouldBe(0);
        recovery.ShouldBeGreaterThan(provenance);
        heading.ShouldBeGreaterThan(recovery);

        block.ShouldContain("install the tool with `dotnet tool install -g Zphil.LoadBearing.Cli`");
        block.ShouldContain("`dotnet dnx Zphil.LoadBearing.Cli --yes -- check <solution>`");

        // Anchored on the backtick that opens the command, because `dotnet dnx` contains the substring
        // the naive form would match.
        block.ShouldNotContain("`dnx ");
    }

    [Fact]
    public void ScopeCard_CanonicalQuarantinedScope_MatchesGolden()
    {
        ArchRule containment = Checker.Canonical
    ...
    {
        string block = AgentContextRenderer.RootBlock(Checker.Canonical, "Spec");
    ...
    public void RootBlock_WithoutInjectOrRegistered_OmitsBothGlossaryAdditions()
    {
        ...
        string block = AgentContextRenderer.RootBlock(DogfoodShapeModel, "Zphil.LoadBearing.ArchSpec");
        block.ShouldNotContain("inject = a source-level constructor-parameter dependency");
        block.ShouldNotContain("registered = named in a source-level container registration");
    }
