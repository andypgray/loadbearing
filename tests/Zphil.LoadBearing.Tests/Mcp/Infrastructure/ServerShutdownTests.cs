...
public sealed class ServerShutdownTests : IDisposable
{
    ...
    {
        // Arrange — a disposer that throws must not deadlock or skip exit. The contract:
    ...
    {
        // A mid-flight tool call must finish before shutdown runs disposers or exits, so
        // Environment.Exit can't tear a non-atomic operation. Enter a call, start the shutdown on a
        // background thread, prove it is blocked (nothing disposed, not exited), then complete the call
        // and prove it proceeds.
    ...
    {
        // Bound — a call that never completes must not hold shutdown hostage. With the in-flight count
