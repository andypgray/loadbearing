...
///     workspace-load diagnostics and/or synthetic failed projects — the one way to drive the
///     incomplete-model gate over a fixture that loads perfectly well. Shared by the gate tests of every verb
///     that has one, so <c>check</c>, <c>baseline</c>, <c>status</c> and <c>graph</c> are exercised against
///     the same injected input rather than each inventing its own.
/// </summary>
/// <remarks>
///     <para>
///         <b>The two inputs are separate on purpose, because the product separates them.</b> Diagnostics
///         render and never gate; failed projects gate and are what every refusal names. A caller that wants
///         a refusal injects a project, a caller that wants a rendered-but-harmless warning injects a
///         diagnostic, and a caller testing that the two do not leak into each other injects one without the
///         other — which is the shape both localized-message defects took in the field.
///     </para>
///     <para>
