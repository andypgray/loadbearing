...
- `layering/adapter-is-a-leaf` — The Adapter layer must not be referenced by the Core layer or the Extraction layer. The adapter rides xunit.v3; a product reference would ship a test framework to every consumer of the referencing package. Host and Pack are the other two leaves, and layering/leaves-independent already holds all three off each other, so this rule names only what sits below the adapter.
