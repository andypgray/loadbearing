...
///     <c>LOADBEARING_DISABLE_PARENT_WATCH=true</c> to opt out (for debugging or detached runs).
/// </remarks>
internal static class ParentProcessWatcher
{
    ...
    internal static void Start(Func<int?> parentLookup, Action onParentExited, bool disabled)
    {
        if (disabled)
        {
            Log.Information("Parent process watcher disabled via LOADBEARING_DISABLE_PARENT_WATCH");
            return;
        }
