...
- A failed project load makes the model wrong, not smaller: `arch_graph` errors naming the projects (a build precondition, not a fault); `arch_check`/`arch_status` return their document stamped `modelIncomplete: true` + `failedProjects` — report that, never plain green.
