...
- `arch_context` — return the frozen-scope card(s) covering a path (dragons prose + sanctioned surface), or a pointer line when none apply. Call before editing an unfamiliar directory to learn if it is dragon territory. Params: `path` (a file or directory, absolute or solution-relative).
