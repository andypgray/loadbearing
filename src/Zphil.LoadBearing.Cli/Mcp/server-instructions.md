...
- `arch_check` — run the whole spec against the solution and return the JSON check report (schemaVersion 3). Call before finishing work that touched architecture-relevant code, to confirm no rule went red. Params: `diffBase` (optional git ref; changed files in a quarantined scope raise a tripwire warning).
