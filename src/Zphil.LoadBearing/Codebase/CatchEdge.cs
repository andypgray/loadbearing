...
///     the <c>catch</c> clauses, deduped by (file, line), and <see cref="UnfilteredSites" /> the subset of
///     those whose clause spells no <c>when</c> filter.
/// </summary>
/// <remarks>
...
{
    internal CatchEdge(
        TypeNode source, TypeNode caught, IReadOnlyList<SourceLocation> sites, IReadOnlyList<SourceLocation> unfilteredSites)
    {
        Source = source;
        Caught = caught;
        Sites = sites;
