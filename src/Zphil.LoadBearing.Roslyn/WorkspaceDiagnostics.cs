...
///         failure). So the decision moved off the text entirely:
///         <see cref="ProjectLoadFailures" /> reads which projects failed off the loaded solution's own
///         structure, and this type carries that answer to every surface. The diagnostics render; they
///         decide nothing.
///     </para>
///     <para>
///         <b>Why one type rather than three lists.</b> The streams must not be swapped: the gate keys
///         strictly on the failed projects, while the rendered stream carries the load diagnostics, the
    ...
    IReadOnlyList<string> UncheckedProjects)
{
    ...
    ///     and it decides no verdict. What gates is <see cref="FailedProjects" />, which no message text can
    ///     influence in any language.
    /// </remarks>
    ...
    /// <summary>
    ///     Whether the model is incomplete — at least one project failed to load — independently of whether
    ///     the operator opted in. This is the fact the JSON documents carry: the opt-out changes the exit
    ///     code, never the truth about the model.
    /// </summary>
    internal bool IsIncomplete => FailedProjects.Count > 0;
