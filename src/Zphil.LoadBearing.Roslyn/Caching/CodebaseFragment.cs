...
///     externals it references, its reference edges, and its member-use edges (GRAMMAR §4.5). It holds no
///     Roslyn types (no <c>ISymbol</c>, <c>Compilation</c>, or <c>Location</c>) so it is
///     System.Text.Json-serializable by design — the persisted extraction cache (Phase 11 WP6) stores
///     exactly this — and
...
///     SymbolId) so serialization is stable; global ordering is re-derived at merge, so a fragment's
///     internal order never affects the model.
/// </remarks>
